...
describe("VaultBTC", function () {
  ...
  describe("Minting", function () {
    it("Should allow anyone to mint tokens", async function () {
      ...
      await vaultBTC.mint(user1.address, mintAmount);
    ...
    it("Should emit Transfer event on mint", async function () {
      ...
      await expect(vaultBTC.mint(user1.address, mintAmount))
    ...
    it("Should allow multiple mints", async function () {
      ...
      await vaultBTC.mint(user1.address, mint1);
      await vaultBTC.mint(user2.address, mint2);
  ...
  describe("Burning", function () {
    ...
    it("Should allow user to burn their own tokens", async function () {
      ...
      await vaultBTC.connect(user1).burn(user1.address, burnAmount);
    ...
    it("Should emit Transfer event on burn", async function () {
      ...
      await expect(vaultBTC.connect(user1).burn(user1.address, burnAmount))
        .to.emit(vaultBTC, "Transfer")
    ...
    it("Should allow burning with allowance", async function () {
      ...
      await vaultBTC.connect(user1).approve(user2.address, burnAmount);
      
      // user2 burns user1's tokens
      await vaultBTC.connect(user2).burn(user1.address, burnAmount);
    ...
    it("Should revert when burning more than balance", async function () {
      ...
      await expect(
        vaultBTC.connect(user1).burn(user1.address, excessiveAmount)
      ).to.be.reverted;
    });
    ...
    it("Should revert when burning without sufficient allowance", async function () {
      ...
      await expect(
        vaultBTC.connect(user2).burn(user1.address, burnAmount)
      ).to.be.reverted;
    });
  });
